...
def make_plot(common_args, goal_args, labels):
    ...
    if labels[0] == "Budget":
        ax.fill_between(
            x, y1, y2, where=(y2 > y1), color="r", alpha=0.2, interpolate=True
        )
        ax.fill_between(
            x, y2, y1, where=(y2 <= y1), color="g", alpha=0.2, interpolate=True
        )
    else:
        ax.fill_between(
            x, y1, y2, where=(y2 < y1), color="r", alpha=0.2, interpolate=True
        ...
        ax.fill_between(
            x, y1, y2, where=(y2 > y1), color="g", alpha=0.2, interpolate=True
        )
